...
class TalkingHeadsDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> dict:
        ...
        vid_lat = torch.from_numpy(vid_lat).squeeze(0).float()  # (16, T, 96, 96)
        T_total = vid_lat.shape[1]

        # Random clip of clip_frames
        start = torch.randint(0, max(1, T_total - self.clip_frames), (1,)).item()
        vid_lat = vid_lat[:, start:start + self.clip_frames, :, :]  # (16, T, 96, 96)

        # Pad if shorter than clip_frames
        ...
        kp_frames = []
        for frame_dict in pose_data[start:start + self.clip_frames]:
        ...
        if self.normalize_kp:
            keypoints = keypoints / self.CANVAS
